...
splits.py
----------
Shared session-based train/validation splitting logic. Splits by
whole recording SESSION (not by individual window), since windows
from the same session overlap heavily (sliding stride) and a random
window-level split would let a model "see" near-duplicates of
